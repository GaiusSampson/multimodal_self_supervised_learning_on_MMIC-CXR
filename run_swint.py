...
import numpy as np
from zero_shot_biobert import make_true_labels as make_true_labels_biobert, run_softmax_eval as run_softmax_eval_biobert
...
def parse_args():
    ...
    parser.add_argument('--batch_size', type=int, default=32, help='the actual batch size in memory')
    parser.add_argument('--effective_batch_size', type=int, default=96, help='the effective batch size make this a multiple of the batch size ' \
    'for best results keep it between 1x and 4x batch size')
    parser.add_argument('--weight_decay', type=float, default=0.0001)
    parser.add_argument('--epochs', type=int, default=6)
    parser.add_argument('--lr', type=float, default=1e-4)
    parser.add_argument('--minimum_lr', type=float, default=1e-6, help='the final learning rate when scheduling, set the same as lr to not use scheduling')
...
def model_pipeline(config, verbose=0): 
    ...
    model, data_loader, device, criterion, optimizer, minimizer, schedular = make(config)

    # and use them to train the model
    train(model, data_loader, device, criterion, optimizer, config, writer, minimizer, schedular)
...
def make(config): 
    ...
    if config.optimizer == "adam": 
        optimizer = optim.AdamW(parameters, lr=config.lr, weight_decay=config.weight_decay)
...
def train(model, loader, device, criterion, optimizer, config, logger, minimizer=None, scheduler=None):
    """
    ...
    if args.use_biobert:
        ...
        y_true = make_true_labels_biobert("test_data/val.csv", cxr_labels)
    else:
        test_loader = load_test_data("test_data/chexpert_val.h5")
    ...
    for epoch in range(config.epochs):
        ...
        for data in tqdm(loader, desc=f"Epoch {epoch+1}"):
            ...
            loss = train_batch(
                config, images, texts, model, device, criterion, optimizer,
            ...
            if (batch_ct % config.log_interval) == 0:
                train_log(running_loss / report_freq, example_ct, batch_ct, epoch, logger)
                running_loss = 0.0
            ...
            if (batch_ct % config.save_interval) == 0 and test_loader is not None:
                ...
                try:
                    with torch.no_grad():
                        if args.use_biobert:
                            y_pred = run_softmax_eval_biobert(
...
def train_batch(config, images, texts, model, device, criterion, optimizer, 
                accumulation_steps=1, minimizer=None):
    """
    Train on a single batch with optional gradient accumulation.
    
    Args:
        config: Training configuration
        images: Batch of images
        texts: Batch of text (strings or tokens)
        model: CLIP model
        device: torch device
        criterion: Loss function
        optimizer: Optimizer
        accumulation_steps: Number of batches to accumulate before stepping
        minimizer: Optional ASAM minimizer
    ...
    # Handle ASAM minimizer (if using)
